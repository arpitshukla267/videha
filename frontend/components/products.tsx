...
// Accept optional products from API; fall back to static PRODUCTS
type ProductsProps = {
  products?: ApiProduct[];
};
...
export function Products({ products }: ProductsProps = {}) {
  ...
  // Use API products if provided, otherwise fall back to static data
  const productList: Array<{ name: string; image: string; copy: string }> =
    products && products.length > 0
      ? products
      : PRODUCTS.map((p) => ({ name: p.name, image: p.image, copy: p.copy }));
