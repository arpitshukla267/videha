...
/* ------------------------------------------------------------------ */
function DesktopHero({ stories }: { stories: Story[] }) {
  const STORIES = stories;
  const SEGMENTS = Math.max(1, STORIES.length - 1);
  ...
  return (
    <>
      ...
      <div className="mt-9 flex flex-wrap gap-3">
        ...
        >
          {story.ctaLabel || "Request Quotation"}
...
function MobileHero({ stories }: { stories: Story[] }) {
...
export function Hero({ stories }: { stories?: Story[] }) {
  const list = stories && stories.length > 0 ? stories : STORIES;
  return (
    <section id="top" className="relative w-full bg-background">
      <DesktopHero stories={list} />
      <MobileHero stories={list} />
    </section>
  );
}
