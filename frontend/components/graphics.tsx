...
/*
  5. Business Registrations Graphic
  Displays only confirmed Indian export registrations — not product/system certifications.
*/
export function ComplianceBadgesGraphic({ className = "" }: { className?: string }) {
  const badges = [
    { label: "IEC", sub: "Import Export Code" },
    { label: "GST", sub: "Goods & Services Tax" },
    { label: "FSSAI", sub: "Food Safety License" },
    { label: "APEDA / RCMC", sub: "Export Registration" },
    { label: "CIN", sub: "Corporate Identity" },
  ]

  return (
    <div className={`grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-5 ${className}`}>
      {badges.map((badge) => (
        <div
          key={badge.label}
          className="flex flex-col items-center justify-center p-5 border border-border/80 bg-background text-center hover:border-primary hover:shadow-[0_12px_28px_rgba(0,0,0,0.06)] transition-all duration-300 rounded-xl"
        >
          <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-full border border-primary/20 bg-primary/5 text-xs font-bold font-mono text-primary">
            {badge.label.split(" ")[0]}
          </div>
