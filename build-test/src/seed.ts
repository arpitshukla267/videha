...
import { assertCloudinaryForSeed, mapInBatches, uploadSeedMedia, uploadSeedPdf } from "./lib/seed-media";
import { SEED_UPLOAD_BATCH_SIZE } from "./lib/cloudinary";
...
async function hydrateSeedMedia() {
  ...
  const products = await mapInBatches(PRODUCTS_SEED, SEED_UPLOAD_BATCH_SIZE, async (product) => ({
    ...product,
    image: await uploadSeedMedia("products", product.slug, "main", product.image),
  }));

  console.log("\nHero stories");
  const heroStories = await mapInBatches(HERO_SEED, SEED_UPLOAD_BATCH_SIZE, async (story) => ({
    ...story,
  ...
  console.log("\nProcess steps");
  const processSteps = await mapInBatches(PROCESS_STEPS_SEED, SEED_UPLOAD_BATCH_SIZE, async (step) => ({
    ...step,
