...
const HeroStory_1 = require("./models/HeroStory");
const SiteContent_1 = require("./models/SiteContent");
const seed_media_1 = require("./lib/seed-media");
const cloudinary_1 = require("./lib/cloudinary");
function processStepId(num, label) {
...
async function hydrateSeedMedia() {
    await (0, seed_media_1.assertCloudinaryForSeed)();
    console.log("Products");
    const products = await (0, seed_media_1.mapInBatches)(PRODUCTS_SEED, cloudinary_1.SEED_UPLOAD_BATCH_SIZE, async (product) => ({
        ...product,
        image: await (0, seed_media_1.uploadSeedMedia)("products", product.slug, "main", product.image),
    }));
    console.log("\nHero stories");
    const heroStories = await (0, seed_media_1.mapInBatches)(HERO_SEED, cloudinary_1.SEED_UPLOAD_BATCH_SIZE, async (story) => ({
        ...story,
        image: await (0, seed_media_1.uploadSeedMedia)("hero", story.id, "desktop", story.image),
        mobileImage: story.mobileImage
            ? await (0, seed_media_1.uploadSeedMedia)("hero", story.id, "mobile", story.mobileImage)
            : undefined,
    }));
    console.log("\nProcess steps");
    const processSteps = await (0, seed_media_1.mapInBatches)(PROCESS_STEPS_SEED, cloudinary_1.SEED_UPLOAD_BATCH_SIZE, async (step) => ({
        ...step,
        image: await (0, seed_media_1.uploadSeedMedia)("process-steps", processStepId(step.num, step.label), "image", step.image),
    }));
    console.log("\nBrochure");
    const siteConfig = await Promise.all(SITE_CONFIG_SEED.map(async (entry) => {
        if (entry.key !== "brochure" || typeof entry.value !== "object" || !entry.value) {
            return entry;
        }
        const brochure = entry.value;
        return {
            ...entry,
            value: {
                ...brochure,
                url: await (0, seed_media_1.uploadSeedPdf)("brochure", brochure.url),
            },
        };
    }));
    return {
...
const SITE_CONFIG_SEED = [
    ...
    },
    {
        key: "registrations",
        value: {
            disclaimer: "We do not display product or system certifications (such as ISO, HACCP, Organic, Halal, Kosher, or US FDA registration) unless a valid, current certificate has been confirmed and supplied by Videha Overseas.",
            items: [
...
async function seed() {
    ...
    await Promise.all([
        ...
        HeroStory_1.HeroStory.deleteMany({}),
        SiteContent_1.ProcessStep.deleteMany({}),
        SiteContent_1.QualityPoint.deleteMany({}),
        SiteContent_1.Market.deleteMany({}),
        SiteContent_1.Service.deleteMany({}),
        SiteContent_1.BuyerExpectation.deleteMany({}),
        SiteContent_1.IntroFact.deleteMany({}),
        SiteContent_1.SiteConfig.deleteMany({}),
    ]);
    console.log("Cleared existing data");
    const media = await hydrateSeedMedia();
    // Insert all
    await Product_1.Product.insertMany(media.products);
    console.log(`\n✓ Seeded ${media.products.length} products`);
    await HeroStory_1.HeroStory.insertMany(media.heroStories);
    console.log(`✓ Seeded ${media.heroStories.length} hero stories`);
    await SiteContent_1.ProcessStep.insertMany(media.processSteps);
    console.log(`✓ Seeded ${media.processSteps.length} process steps`);
    await SiteContent_1.QualityPoint.insertMany(QUALITY_POINTS_SEED);
    console.log(`✓ Seeded ${QUALITY_POINTS_SEED.length} quality points`);
    await SiteContent_1.Market.insertMany(MARKETS_SEED);
    console.log(`✓ Seeded ${MARKETS_SEED.length} markets`);
    await SiteContent_1.Service.insertMany(SERVICES_SEED);
    console.log(`✓ Seeded ${SERVICES_SEED.length} services`);
    await SiteContent_1.BuyerExpectation.insertMany(BUYER_EXPECTATIONS_SEED);
    console.log(`✓ Seeded ${BUYER_EXPECTATIONS_SEED.length} buyer expectations`);
    await SiteContent_1.IntroFact.insertMany(INTRO_FACTS_SEED);
    console.log(`✓ Seeded ${INTRO_FACTS_SEED.length} intro facts`);
    await SiteContent_1.SiteConfig.insertMany(media.siteConfig);
