...
import { QUALITY_POINTS } from "@/lib/content"

type QualityProps = {
  preview?: boolean
}

export function Quality({ preview = false }: QualityProps) {
  return (
            ...
            {QUALITY_POINTS.map((p, i) => (
              <Reveal key={p.title} delay={0.05 + i * 0.05}>
                <div className="border-t border-background/15 py-5 pr-4">
                  ...
                  <h3 className="mt-2 text-base font-medium text-background">{p.title}</h3>
