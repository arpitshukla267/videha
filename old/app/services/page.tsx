...
import { ArrowRight, CheckCircle2, ShieldCheck, Layers, FileText, ChevronRight, Check } from "lucide-react"
    ...
    <main className="overflow-hidden bg-background">
      ...
      <header className="relative border-b border-border pt-36 md:pt-48 pb-20 md:pb-28 overflow-hidden">
        ...
        <div className="relative z-10 mx-auto max-w-[1400px] px-5 md:px-10">
          ...
          <h1 className="mt-6 max-w-4xl text-[clamp(2.5rem,6.5vw,5rem)] font-semibold leading-[0.98] tracking-[-0.03em] text-foreground text-balance">
            Built for Buyers Who Think Beyond the Shipment.
          </h1>
          <Reveal delay={0.1}>
            <p className="mt-6 max-w-2xl text-[16px] leading-relaxed text-muted-foreground">
              Videha Overseas is a dedicated supply chain partner. We coordinate procurement, processing, grading, packaging, and logistics to make importing premium Indian makhana reliable and risk-free.
            </p>
          </Reveal>
        </div>
      </header>
      ...
      <section className="py-24 md:py-32 border-b border-border bg-background">
        <div className="mx-auto max-w-[1400px] px-5 md:px-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
            ...
            <div className="lg:col-span-6 flex flex-col gap-2">
              ...
              {SERVICES_DATA.map((service, idx) => (
                <button
                  ...
                  className={`text-left border-b border-border/60 py-6 pr-4 flex items-start gap-4 transition-all duration-300 ${
                    activeIndex === idx
                ...
                >
                  ...
                  <div className="flex-1">
                    <h3 className={`text-lg md:text-xl font-semibold tracking-tight transition-colors ${
                      activeIndex === idx ? "text-primary font-bold" : "text-foreground"
                    }`}>
                      {service.title}
                    </h3>
                  ...
                  <ChevronRight className={`w-5 h-5 mt-1 transition-transform ${
                    activeIndex === idx ? "text-primary translate-x-1" : "text-muted-foreground/30"
                  }`} />
                </button>
              ))}
            </div>
            ...
            {/* Right: Preview Pane */}
            <div className="lg:col-span-6 sticky top-28 border border-border bg-background p-6 md:p-10 shadow-xs">
              <AnimatePresence mode="wait">
                ...
                >
                  <div className="relative aspect-[16/10] w-full overflow-hidden border border-border bg-secondary">
                    ...
                    />
                    <div className="absolute top-3 right-3 bg-background/90 px-3 py-1 text-[9px] font-mono uppercase text-foreground border border-border">
                      SERVICE {SERVICES_DATA[activeIndex].num}
                    </div>
                  </div>
        ...
        <div className="mx-auto max-w-[1400px] px-5 md:px-10">
          <div className="text-center max-w-2xl mx-auto mb-16">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              A structured operational timeline designed to eliminate friction in procurement.
            </p>
          </div>
          ...
          <div className="relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6 relative z-10">
              {FLOW_STEPS.map((step, idx) => (
                <div key={step.name} className="bg-background border border-border p-6 flex flex-col justify-between h-full group hover:border-primary transition-all duration-300">
                  <div>
                    ...
                    <h3 className="text-sm font-bold text-foreground uppercase tracking-tight">{step.name}</h3>
        ...
        <div className="mx-auto max-w-[1400px] px-5 md:px-10">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="p-8 border border-border bg-background">
              ...
              <h3 className="text-lg font-bold text-foreground">Moisture Content Guarantee</h3>
              <p className="text-xs text-muted-foreground mt-2 leading-relaxed">
                If the moisture content in popped shipments exceeds the contracted 4.5% limit upon container departure, we offer replacement lots immediately.
              </p>
            </div>

            <div className="p-8 border border-border bg-background">
              <Layers className="w-8 h-8 text-primary mb-4" />
              <h3 className="text-lg font-bold text-foreground">Custom Packaging Adaptability</h3>
              <p className="text-xs text-muted-foreground mt-2 leading-relaxed">
                We format retail prints, box sizing, nitrogen levels, and outer export carton densities to match exact regulations of the target port.
              </p>
            </div>

            <div className="p-8 border border-border bg-background">
              <FileText className="w-8 h-8 text-primary mb-4" />
              <h3 className="text-lg font-bold text-foreground">In-House Export Bureau</h3>
              <p className="text-xs text-muted-foreground mt-2 leading-relaxed">
                We handle land freight, customs inspections, and global sea lines to deliver complete door-to-port or door-to-door transit logistics.
              </p>
            </div>
          </div>
        </div>
