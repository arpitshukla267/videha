...
import { ArrowRight, CheckCircle2, ShieldCheck, Sprout, Award, MapPin } from "lucide-react"
import { SectionLabel } from "@/components/section-label"
import { Reveal } from "@/components/reveal"
import { OriginSealGraphic, TraceabilityFlowGraphic, ComplianceBadgesGraphic } from "@/components/graphics"
...
const PRINCIPLES = [
  {
    ...
    description: "Every shipment is traceable back to specific harvesting clusters in the wetlands of Mithila, Bihar. By bypassing raw material traders and working directly with local farming cooperatives at origin, we secure first-grade seed lots and guarantee batch purity.",
  ...
  },
  {
    ...
    description: "Export makhana demands absolute physical consistency. We sort all popped lots mechanically and optically to ensure only 6mm+ Super Jumbo puffs make it into our packing lines. Unpopped kernels, shell fragments, and off-color puffs are strictly weeded out.",
  ...
  },
  {
    ...
    description: "Popped makhana is highly hygroscopic, easily absorbing moisture and turning soft. To lock in the delicate crunch across 20+ days of ocean transit, we kiln-dry our makhana to under 4.5% moisture and seal it immediately in heavy-gauge double-barrier packaging.",
  ...
  },
  {
    ...
    description: "We prepare all phytosanitary certifications, commercial invoices, and origin certificates with meticulous care. Every container comes with an independent lab Certificate of Analysis (COA) matching destination market regulations.",
...
export default function AboutPage() {
  return (
    <main className="overflow-hidden bg-background">
      {/* HERO SECTION */}
      <header className="relative border-b border-border pt-36 md:pt-48 pb-16 overflow-hidden">
        <Image
        ...
        <div className="relative z-10 mx-auto max-w-[1400px] px-5 md:px-10">
          <div className="max-w-4xl">
            ...
            <h1 className="mt-6 text-[clamp(2.5rem,6.5vw,5.2rem)] font-semibold leading-[0.98] tracking-[-0.03em] text-foreground text-balance">
              Built in India.<br />Ready for the World.
            </h1>
            <Reveal delay={0.1}>
              <p className="mt-8 max-w-2xl text-[16px] leading-relaxed text-muted-foreground">
                Videha Overseas bridges India&apos;s ancient wetland heritage with the strict quality and supply expectations of international markets — delivering premium, export-graded fox nuts to discerning global buyers.
              </p>
            </Reveal>
          </div>
        </div>
      ...
      {/* HERO IMAGE SECTION (Cinematic crop) */}
      <section className="">
        <div className="mx-auto max-w-[1400px] px-5 pb-20 md:px-10 md:pb-28">
          <Reveal>
            <div className="relative aspect-[21/9] w-full overflow-hidden border border-border bg-secondary">
              <Image
                src="/images/intro-farm.png"
                alt="Lotus cultivation in Mithila, Bihar, India"
                fill
                className="object-cover object-center"
                sizes="100vw"
                priority
              />
              {/* <div className="absolute top-4 left-4 bg-background/90 backdrop-blur-xs px-3 py-1.5 border border-border text-[11px] font-mono uppercase tracking-wider text-foreground flex items-center gap-1.5">
                <MapPin className="w-3.5 h-3.5 text-accent" />
                Origin Cultivation, Bihar
              </div> */}
            </div>
          </Reveal>
        </div>
          ...
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-16">
            <div className="lg:col-span-5">
              <Reveal>
                ...
                <h2 className="mt-2 text-3xl font-semibold tracking-tight text-foreground md:text-4xl leading-tight">
                  Our Origin & Wetland Heritage
                </h2>
              </Reveal>
            </div>
            <div className="lg:col-span-7">
              <Reveal delay={0.05}>
                <p className="text-[clamp(1.25rem,2.5vw,1.8rem)] font-normal leading-snug tracking-tight text-foreground/90 text-balance">
                  Bihar&apos;s Mithila wetlands produce over 80% of the world&apos;s premium fox nuts. We build direct procurement partnerships at origin to ensure stable crop supply and unmatched physical grade purity.
                </p>
              </Reveal>
              <Reveal delay={0.1}>
                <p className="mt-6 text-[15px] leading-relaxed text-muted-foreground">
                  Makhana harvesting is a highly specialized craft passed down through generations. Our farmers harvest water lily seeds from shallow wetland ponds during late summer. By working directly with these farming clusters, Videha Overseas ensures fair compensation at the source while maintaining strict physical checks from the moment seeds leave the water.
                </p>
              </Reveal>
            </div>
          </div>
      ...
      <section className="border-b border-border bg-[#f8f6f0] py-24 md:py-32">
        <div className="mx-auto max-w-[1400px] px-5 md:px-10">
          ...
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-16 items-center">
            ...
            {/* Right: Text details, geographic badge and smaller supporting image */}
            <div className="lg:col-span-6 flex flex-col gap-8">
              <div className="flex flex-col md:flex-row items-start gap-6 border-b border-border pb-8">
                <div className="shrink-0 p-4 bg-background border border-border flex flex-col items-center">
                  <OriginSealGraphic className="w-20 h-20" />
                  <span className="text-[9px] font-mono uppercase tracking-wider text-accent mt-2">
                    Mithila Origin
                  </span>
                </div>
                <div>
                  <h3 className="text-xl font-semibold text-foreground">A Natural Superfood</h3>
                  <p className="mt-2 text-sm text-muted-foreground leading-relaxed">
                    Euryale ferox, commonly known as fox nut or makhana, matures in wetlands before its black seeds are popped under extreme heat. The resulting white puff is high in protein, fiber, and minerals while remaining naturally gluten-free and low-glycemic.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-center">
                ...
                <div className="text-xs text-muted-foreground leading-relaxed">
                  Our grading system separates raw seeds into precise size brackets (ranging from Super Jumbo 6.5mm+ to standard grades) to ensure that the expansion ratio and crunch density are optimized during popping.
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mx-auto max-w-[1400px] px-5 md:px-10">
          <div className="mb-16 text-center max-w-2xl mx-auto">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              A seamless, audited supply line built specifically for international distribution.
            </p>
          </div>

          <div className="relative">
            {/* SVG Connecting Line for Desktop */}
            <div className="hidden lg:block absolute left-4 right-4 top-1/2 -translate-y-1/2 h-0.5 bg-border z-0" />

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-8 relative z-10">
              {TIMELINE_FLOW.map((t, idx) => (
                <Reveal key={t.step} delay={idx * 0.05}>
                  <div className="flex flex-col bg-background border border-border p-6 h-full hover:border-primary transition-colors">
                    <div className="flex items-center justify-between border-b border-border pb-4 mb-4">
                      <span className="font-mono text-xs font-bold text-accent">STEP {t.step}</span>
                      <span className="text-[9px] font-mono uppercase bg-secondary px-2 py-0.5 border border-border">
                        {t.label}
                      </span>
                    </div>
                    <h3 className="text-sm font-bold text-foreground leading-tight">{t.title}</h3>
                    <p className="mt-2 text-[11px] text-muted-foreground leading-relaxed flex-1">
                      {t.body}
                    </p>
                  </div>
                </Reveal>
        ...
        <div className="relative z-10 mx-auto max-w-[1400px] px-5 md:px-10">
          <div className="max-w-4xl">
            ...
            <p className="mt-8 max-w-xl text-[14px] leading-relaxed text-background/60">
              Foreign importers, health food brands, and distributors cannot afford variable quality or container-to-container delays. We back every order with locked pricing contracts, strict quality grades, and transparent EXIM clearance.
            </p>
          </div>
        </div>
      </section>
            ...
            <div className="p-8 md:p-16 border border-border bg-background flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-2 max-w-xl">
                  Contact our export team for sample requests, specifications sheets, and container pricing.
                </p>
              </div>
